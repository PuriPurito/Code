                    ...
                    <div class="document__buttons">
                        ...
                        <%
                                ...
                                if (curObject.request_type_id.ForeignElem.code == 'afl_acquaint') {
                                  oAcquaintElem = curObject.custom_elems.GetOptChildByKey('acquaint');
                                  if (oAcquaintElem != undefined) {
                                    docAcquaint = tools.open_doc(oAcquaintElem.value);
                                    if (docAcquaint != undefined) {
                                      //oSignature = ArrayOptFirstElem(docAcquaint.TopElem.signature_files);
                                      iPrintFormID = undefined;
                                      oCourseIdElem = docAcquaint.TopElem.custom_elems.GetOptChildByKey('course_id');
                                      if (oCourseIdElem != undefined) {
                                        docCourse = tools.open_doc(oCourseIdElem.value);
                                        if (docCourse != undefined) {
                                          oPrintFormElem = docCourse.TopElem.custom_elems.GetOptChildByKey('print_form');
                                          if (oPrintFormElem != undefined) {
                                            iPrintFormID = OptInt(oPrintFormElem.value)
                                          }
                                        }
                                      }
                                      if (iPrintFormID != undefined) {
                                        slinkToSave = "/view_print_form.html?print_form_id="+iPrintFormID+"&object_id="+docAcquaint.TopElem.id+"&sid="+tools_web.get_sum_sid(iPrintFormID, Request.Session.sid);
                                      }
                                    }
                                  }
                                }
