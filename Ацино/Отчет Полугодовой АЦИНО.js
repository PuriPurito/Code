var sLogName = "KD_ACINO_SEMIANNUAL_REPORT_LOG";
...
function main() {
	...
	if (docAssessmentAppraise != undefined) {
		...
		for (oPlan in arPlans) {
			oPA = ArrayOptFindByKey(arPAs, OptInt(oPlan.person_id), "person_id");
			if (oPA != undefined) {
				...
				if (docPA != undefined) {
					...
					if (docPlan != undefined) {
						...
						if (iWorkflowStateNumber < 2) {
							oListElem.status = "Не начато";
						}
						if (iWorkflowStateNumber > 2 || sWorkflowState == "2_end") {
							oListElem.status = "Одобрено менеджером";
						}
						if (sWorkflowState == "2_boss") {
							oListElem.status = "На одобрении менеджера";
						}
						if (sWorkflowState == "2_person") {
							if (ArrayOptFind(docPlan.TopElem.workflow_log_entrys, "This.begin_state == '2_boss' && This.finish_state == '2_person'") != undefined) {
						...
						oLogPersonToBoss = ArrayOptFind(docPlan.TopElem.workflow_log_entrys, "This.begin_state == '2_person' && This.finish_state == '2_boss'");
						...
						oLogBossToEnd = ArrayOptFind(docPlan.TopElem.workflow_log_entrys, "This.begin_state == '2_boss' && This.finish_state == '2_end'");
...
try {
	AlertLog("Формирование Отчета Ацино Полугодовой начато");
	arRes = main();
	AlertLog("Формирование Отчета Ацино Полугодовой завершено");
}
catch (e) {
	AlertLog(e);
	arRes = [];
