...
var arPAs = ArraySelectAll(XQuery("for $elem in pas where assessment_appraise_id = " + iAssessmentAppraise + " and assessment_appraise_type = 'development_plan' return $elem"));
var arResult = [];
            ...
            if (docPA != undefined) {
                for (oTask in docPA.TopElem.tasks) {
                    ...
                    if (docTask != undefined) {
                        ...
                        oIsCompetence = ArrayOptFindByKey(docTask.TopElem.custom_fields, "competence_id", "name");
                        if (oIsCompetence != undefined) {
                            oListElem.CompetenceID = oIsCompetence.value;
                        }
                        oLearningByWork = ArrayOptFindByKey(docTask.TopElem.custom_fields, "learning_by_work", "name");
                        if (oLearningByWork != undefined) {
                            oListElem.LearningByWork = oLearningByWork.value;
                        }
                        oLearningByColleagues = ArrayOptFindByKey(docTask.TopElem.custom_fields, "learning_by_colleagues", "name");
                        if (oLearningByColleagues != undefined) {
                            oListElem.LearningByColleagues = oLearningByColleagues.value;
                        }
                        oProgramID1 = ArrayOptFindByKey(docTask.TopElem.custom_fields, "program_id1", "name");
                        if (oProgramID1 != undefined) {
                            oListElem.ProgramID1 = oProgramID1.value;
                        }
                        oProgramID2 = ArrayOptFindByKey(docTask.TopElem.custom_fields, "program_id2", "name");
                        if (oProgramID2 != undefined) {
                            oListElem.ProgramID2 = oProgramID2.value;
                        }
                        oTaskComment = ArrayOptFindByKey(docTask.TopElem.custom_fields, "task_comment", "name");
                        if (oTaskComment != undefined) {
                            oListElem.TaskComment = oTaskComment.value;
                        }
