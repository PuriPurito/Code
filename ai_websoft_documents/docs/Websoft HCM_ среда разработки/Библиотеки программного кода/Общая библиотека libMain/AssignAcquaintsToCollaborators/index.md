## AssignAcquaintsToCollaborators

Метод **AssignAcquaintsToCollaborators** предназначен для назначения ознакомлений сотрудникам.

_Синтаксис (вызов из JS-кода):_  
      **tools.call\_code\_library\_method ("libMain", "AssignAcquaintsToCollaborators", <arrParams>)**
...
\- _arrCollaboratorIDs_ \- массив ID сотрудников (массив целых чисел).  
\- _dNormativeDate_ \- требуемая дата ознакомления (дата).  
\- _strReAssignParam_ – действие после ознакомления пользователя (строка). Допустимые значения: _"skip"(Пропустить); "with\_periodity"(Назначить, если наступил срок); "always"(Назначить в любом случае)_.  
