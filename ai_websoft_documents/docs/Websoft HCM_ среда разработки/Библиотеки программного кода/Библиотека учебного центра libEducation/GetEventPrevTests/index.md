...
Метод предназначен для получения списка предварительных тестов по мероприятию.

_Синтаксис (вызов из JS-кода):_  
      **tools.call\_code\_library\_method ("libEducation", "GetEventPrevTests", <arrParams>)**
...
      Тип: **Объект**. Содержит перечень предварительных тестов по мероприятию, а также ряд дополнительных атрибутов:  
\- _array_ \- массив предварительных тестов, которые были прикреплены к мероприятию (массив объектов).  
...
`_// Вызов метода с помощью функции_ _tools.call_code_library_method_   _oRes = tools.call_code_library_method ("libEducation", "GetEventPrevTests", [OptInt (iEventID)]);   // Проверьте назначение предварительных тестов по мероприятию: Учебный центр - Карточка мероприятия - Контрольные   // Если участникам мероприятия предварительные тесты не назначены, произведите назначение, нажав на кнопку "Назначить предварительные тесты участникам".   // Запись результата в основной журнал (лог) системы xhttp-<текущая_дата>.log   LogEvent('', EncodeJson(oRes));_`
...
`_// Вызов метода с помощью функции_ _tools.call_code_library_method_   _oRes = tools.call_code_library_method ("libEducation", "GetEventPrevTests", [OptInt (iEventID)]);   // Проверьте назначение предварительных тестов по мероприятию: Учебный центр - Карточка мероприятия - Контрольные   // Если участникам мероприятия предварительные тесты не назначены, произведите назначение, нажав на кнопку "Назначить предварительные тесты участникам".   bRESULT = oRes.result;   alert ("Успешность выполнения метода (true/false) = " + bRESULT);   RESULT = oRes.array;   alert ("Количество прикрепленных файлов = " + ArrayCount (RESULT));   for(q in RESULT)   {        str = '';           // @property {bigint} id        // @property {string} name        // @property {number} duration        // @property {string} status        // @property {number} passing_score        // @property {string} link        // @property {string} image_url           res_id = q.id;        str = str + 'res_id = ' + res_id + '\n';         res_name = q.name;        str = str + 'res_name = ' + res_name + '\n';         res_duration = q.duration;        str = str + 'res_duration = ' + res_duration + '\n';        res_status = q.status;        str = str + 'res_status = ' + res_status + '\n';        res_passing_score = q.passing_score;        str = str + 'res_passing_score = ' + res_passing_score + '\n';        res_link = q.link;        str = str + 'res_link = ' + res_link + '\n';         res_image_url = q.image_url;        str = str + 'res_image_url = ' + res_image_url + '\n';           alert ( str );   }   ERROR = oRes.error;   alert ("Код ошибки = " + ERROR);   MESSAGE = oRes.errorText;   alert ("Сообщение об ошибке = " + MESSAGE);_`

_Пример 3:_  
      `_RESULT = tools.call_code_library_method ("libEducation", "GetEventPrevTests", [OptInt (iEventID, curObjectID)]).array;_`
