## GetEducationMethodPostTests

Метод предназначен для получения списка пост-тестов учебной программы.

_Синтаксис (вызов из JS-кода):_  
      **tools.call\_code\_library\_method ("libEducation", "GetEducationMethodPostTests", <arrParams>)**
...
      Тип: **Объект**. Содержит перечень пост-тестов учебной программы, а также ряд дополнительных атрибутов:  
\- _array_ \- массив тестов, массив пост-тестов учебной программы (массив объектов).  
...
     _image\_url_ – относительная ссылка на изображение, связанное с тестом (строка).
...
`_// Вызов метода с помощью функции_ _tools.call_code_library_method_   _oRes = tools.call_code_library_method ("libEducation", "GetEducationMethodPostTests", [OptInt (iEduMethodID)]);      // Проверьте наличие пост-тестов для учебной программы: Учебный центр - Карточка учебной программы - Тестирования   // Если участникам учебной программы пост-тесты не назначены, произведите назначение, нажав на кнопку "Выбрать тесты" в области "Пост-тесты".      // Запись результата в основной журнал (лог) системы xhttp-<текущая_дата>.log   LogEvent('', EncodeJson(oRes));_      _Результат выполнения метода, записанный в лог и преобразованный в формат JSON:_`
...
`_oRes = tools.call_code_library_method ("libEducation", "GetEducationMethodPostTests", [OptInt (iEduMethodID)]);      // Проверьте наличие пост-тестов для учебной программы: Учебный центр - Карточка учебной программы - Тестирования   // Если участникам учебной программы пост-тесты не назначены, произведите назначение, нажав на кнопку "Выбрать тесты" в области "Пост-тесты".      bRESULT = oRes.result;   alert ("Успешность выполнения метода (true/false) = " + bRESULT);   RESULT = oRes.array;   alert ("Количество пост-тестов = " + ArrayCount (RESULT));   for(q in RESULT)   {        str = '';_`
...
_Пример 3:_  
      `_RESULT = tools.call_code_library_method ("libEducation", "GetEducationMethodPostTests", [OptInt (iEduMethodID, curObjectID)]).array;_`
