...
Атрибут объекта. Проверяет, выполняется ли еще запущенный поток.
...
      Тип: **Булево**. Возвращает значение, показывающее, продолжает ли метод выполняться (true – метод продолжает выполняться, false – метод в данный момент не выполняется).
