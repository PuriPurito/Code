...
Флаг, обозначающий, что выполнение метода будет запущено в отдельном потоке, при этом управление будет сразу же возвращено вызывающему коду. При этом, если вызов метода происходит в клиентском приложении, и у метода включен атрибут RUN-UI, откроется немодальное окно, показывающее статус выполнения метода с возможностью нажать кнопку **"Отмена"**.  
Значение по умолчанию определяется атрибутом RUN-ASYNC описания метода.  
Для получения статуса завершения метода из внешнего кода необходимо использовать атрибут IsRunning.

_Синтаксис:_  
      **MethodRunner.RunAsync**

_Возвращаемое значение:_  
      Тип: **Булево**. Значение _true_ демонстрирует, что выполнение метода будет запущено в отдельном потоке и что управление будет сразу же возвращено вызывающему коду.

_Пример 1:_  
      `_runner = new MethodRunner( lib_recruit, 'send_vacancy_create_notif' );         runner.SetArguments ( vacancy );         runner.RunAsync = true;         runner.Run ();_`
