...
elem_Array = ArrayOptFirstElem ( XQuery ( "for $elem in pas where $elem/code = '"+ Trim('{[5]}')+"_goals" +"' return $elem" ) )
if ( elem_Array != undefined ) {
 continueFlag=true; 
}
...
elem_Array = ArrayOptFirstElem ( XQuery ( "for $elem in pas where $elem/code = '"+ Trim('{[5]}')+"_goals" +"' return $elem" ) )
if ( elem_Array == undefined ) {
 alert("Этап 6 Строка "+counter+": Не найдена анкета с кодом " + Trim('{[5]}') +"_goals");
 continueFlag=true; 
}
...
elem_Array = ArrayOptFirstElem ( XQuery ( "for $elem in pas where $elem/code = '"+ Trim('{[5]}')+"_goals" +"' return $elem" ) )
if ( elem_Array == undefined ) {
 alert("Этап 7 Строка "+counter+": Не найдена анкета с кодом " + Trim('{[5]}') +"_goals");
 continueFlag=true; 
}

if( Trim("{[14]}")!="" ) {
  curObject.custom_fields.ObtainChildByKey("is_compulsory").value = (Trim("{[14]}") == "Да") ? true : false;
}
}
...
elem_Array = ArrayOptFirstElem ( XQuery ( "for $elem in pas where $elem/code = '"+ Trim('{[5]}')+"_goals" +"' return $elem" ) )
if ( elem_Array == undefined ) {
 alert("Этап 8 Строка "+counter+": Не найдена анкета с кодом " + Trim('{[5]}') +"_goals");
 continueFlag=true; 
} else {
  ...
  if (_t != undefined ) {
    ...
    curObject.tasks.ObtainChildByKey(_t.id).weight = Trim("{[12]}");
  }
}
